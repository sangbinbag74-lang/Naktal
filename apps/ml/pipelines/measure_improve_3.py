"""박상빈님 5/20 #3 개선 — Optuna 10000 trial × 15 카테고리.

기존 200 trial → 10000 trial = 더 정밀 가중 발견 가능.
4 후보 (B-q70, B-q60, B-q80, M1).
"""
...
def main():
    ...
    print(f"\n[{time.strftime('%H:%M:%S')}] Optuna 10000 trial × 15 카테고리…", flush=True)
    ...
    for cat in big_cats:
        ...
        st.optimize(obj, n_trials=10000, show_progress_bar=False)
        ...
        print(f"  [{cat[:15]:15}] {st.best_value:.3f}", flush=True)

    d, t1, w05, w10 = measure_idx(optuna_pred, np.arange(len(df)))
    score = t1 - d * 0.05
    print(f"\n=== #3 Optuna 10000 trial 결과 ===")
    print(f"  부적격: {d:.2f}% / 1위: {t1:.2f}% / 점수: {score:.3f}")
    print(f"  vs #1 200 trial: 3.557 / vs auto_best_16: 3.577")

    out_path = DATA_DIR / f"improve_3_{int(time.time())}.json"
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump({"score": float(score), "disq": float(d), "top1": float(t1),
                    "cat_weights": cat_weights}, f, indent=2, ensure_ascii=False, default=float)
