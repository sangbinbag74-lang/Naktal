"""박상빈님 5/19 #4 — B-q65/q68/q72/q75 fine quantile 학습.

기존 11 quantile (q05~q95) → q70 주변 fine quantile 4개 추가.
박상빈님 cat3 변형에 fine-tune 옵션 추가.

학습 시간: 약 30분~1시간 (4 × 15분).
"""
import sys, io
try:
...
FINE_QUANTILES = [0.65, 0.68, 0.72, 0.75]
...
def main():
    ...
    for alpha in FINE_QUANTILES:
        q_label = f"q{int(alpha*100):02d}"
        print(f"\n[{time.strftime('%H:%M:%S')}] {q_label} (alpha={alpha}) 학습…", flush=True)
        t0 = time.time()
        model = train_one(alpha, X_train, y_train, X_val, y_val)
        elapsed = time.time() - t0
        out = MODEL_DIR / f"sajung_quantile_{q_label}_new.pkl"
        joblib.dump({
            "model": model, "feature_names": feature_cols,
            "categorical_cols": CATEGORICAL_COLS, "numeric_cols": NUMERIC_COLS,
            "encoders": encoders, "alpha": alpha,
            "model_version": f"sajung-quantile-{q_label}-2026-05-19-fine",
        }, out)
        # test 평가
        ...
        print(f"  완료 ({elapsed:.0f}s, iter {model.best_iteration}) — 부적격 {disq:.2f}% / 1위 {top1:.2f}% / 점수 {top1 - disq*0.05:.3f}", flush=True)
        print(f"  저장: {out.name}")

    print(f"\n[{time.strftime('%H:%M:%S')}] 모든 fine quantile 학습 완료", flush=True)
