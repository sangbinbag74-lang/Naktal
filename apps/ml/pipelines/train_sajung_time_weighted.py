"""박상빈님 5/19 #6 — 시간 가중 재학습 (최근 데이터 가중 ↑).

기존 LightGBM Quantile q70 학습 + sample_weight (시간 비례).
박상빈님 2026 lwlt 변경 (87.745 → 89.745) 반영 가능.

가중치: 2024-2025 데이터 = 2.0x, 2023 = 1.5x, 그 이전 = 1.0x.
학습 시간: 약 30분~1시간.
"""
import sys, io, time
try:
...
def main():
    ...
    # 시간 가중 (deadline 기준)
    df_train["deadline_dt"] = pd.to_datetime(df_train["deadline"])
    df_train["year"] = df_train["deadline_dt"].dt.year
    weights = np.where(df_train["year"] >= 2024, 2.0,
                       np.where(df_train["year"] >= 2023, 1.5, 1.0))
    print(f"  시간 가중 분포: 2024+={np.sum(weights==2.0):,} / 2023={np.sum(weights==1.5):,} / 이전={np.sum(weights==1.0):,}")
    ...
    print(f"\n[{time.strftime('%H:%M:%S')}] 시간 가중 학습 시작 (alpha=0.7)…", flush=True)
    model = lgb.train(
        params, train_set,
    ...
    out = MODEL_DIR / "sajung_quantile_q70_timewt.pkl"
    joblib.dump({
        ...
        "model_version": "sajung-quantile-q70-timewt-2026-05-19",
    }, out)
    ...
    print(f"\n=== #6 시간 가중 q70 test 평가 ===")
    print(f"  부적격: {disq:.2f}% / 1위: {top1:.2f}% / 점수: {top1 - disq*0.05:.3f}")
