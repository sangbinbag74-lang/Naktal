/**
 * ML 학습용 CSV 추출 v3 — openedAt 기반 실제 개찰시점 피처 추가
 *
 * v2 (54 피처) + 신규 6 피처 = 60 피처
 * 신규 피처 (BidResult.openedAt 활용 — 99.9999% 채움):
 *   - opened_month: 실제 개찰 월 (deadline 월과 다를 수 있음)
 *   - opened_weekday: 0~6
 *   - opened_hour: 0~23 (KST 기준)
 *   - opened_season_q: 1~4
 *   - days_deadline_to_open: 마감 → 개찰까지 일수
 *   - is_morning_open: 오전 12시 이전 개찰 여부 (1/0)
 *
 * 출력: apps/ml/data/training_data_v3.csv
 * 실행: pnpm ts-node src/scripts/export-training-data-v3.ts
 */
import { Pool } from "pg";
...
const OUT_PATH = path.resolve(__dirname, "../../../../apps/ml/data/training_data_v3.csv");
const CHUNK = 5000;
  ...
  try {
    ...
    while (true) {
      ...
      for (const r of res.rows) {
        ...
        // 신규 v3 — openedAt 기반 (KST 변환)
        const opened = new Date(r.opened_at);
        const deadline = new Date(r.deadline);
        const openedKst = new Date(opened.getTime() + 9 * 3600 * 1000);
        const openedMonth = openedKst.getUTCMonth() + 1;
        const openedWeekday = openedKst.getUTCDay();
        const openedHour = openedKst.getUTCHours();
        const openedSeasonQ = openedMonth <= 3 ? 1 : openedMonth <= 6 ? 2 : openedMonth <= 9 ? 3 : 4;
        const daysDeadlineToOpen = Math.round((opened.getTime() - deadline.getTime()) / 86400000);
        const isMorningOpen = openedHour < 12 ? 1 : 0;
