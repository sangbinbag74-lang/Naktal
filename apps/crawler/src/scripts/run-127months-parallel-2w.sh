#!/bin/bash
# 127월 재수집 2 worker 병렬 (5 worker 부하 경합 회피)
# Stage 2 잔여 34월 + Stage 3 127월 + Stage 4 verify + sentinel
set -u
...
# Stage 2 (extras) 2 worker — 잔여 34월 (이미 적재된 90 + 3 = 93월 제외)
echo "[$(ts)] === 2단계: extras 2 worker 병렬 시작 (잔여 34월) ===" >> "$LOG"
...
wait
echo "[$(ts)] === 2단계: extras 2 worker 모두 종료 ===" >> "$LOG"

# Stage 3 (bid) 2 worker — 127월 평등 분할 (W1=64, W2=63)
echo "[$(ts)] === 3단계: bid 2 worker 병렬 시작 (127월) ===" >> "$LOG"
...
wait
echo "[$(ts)] === 3단계: bid 2 worker 모두 종료 ===" >> "$LOG"
