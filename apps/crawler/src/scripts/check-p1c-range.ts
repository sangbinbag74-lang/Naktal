...
(async () => {
  ...
  const r = await pool.query(`
    ...
    WHERE "openingDate" >= '2008-01-01' AND "openingDate" < '2014-01-01'
    GROUP BY 1 ORDER BY 1
  `);
  for (const row of r.rows) process.stdout.write(`${row.ym}: ${row.filled}/${row.total} (${((Number(row.filled)/Number(row.total))*100).toFixed(1)}%)\n`);
  await pool.end();
})();
