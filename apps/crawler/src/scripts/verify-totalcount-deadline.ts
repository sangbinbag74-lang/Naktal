/**
 * G2B totalCount vs DB 월별 카운트 비교 (deadline 기준 = inqryDiv=2)
 * verify-totalcount.ts(=공고일자 기준)와 짝.
 * - DB는 deadline 기준이므로 inqryDiv=2가 metric 일치 → 매칭률 100% 근접해야 정상
 * - 80% 미만이면 진짜 누락
 */
import { Pool } from "pg";
...
async function getTotalCount(op: string, ym: string): Promise<number> {
  ...
  // inqryDiv=2 = 마감일시(bidClseDt) 기준
  const u = `${BASE}/${op}?serviceKey=${encodeURIComponent(apiKey)}&inqryDiv=2&inqryBgnDt=${from}&inqryEndDt=${to}&numOfRows=1&pageNo=1&type=json`;
...
(async () => {
  ...
  // deadline metric 일치 → 80% 이상이 정상. 미만은 누락
  ...
  const totalGap = rows.reduce((s, r) => s + Math.max(0, r.api - r.db), 0);

  console.log("=== deadline 기준 검증 (inqryDiv=2, 2002-01 ~ 2026-04) ===");
  console.log(`전체 G2B 합:   ${totalApi.toLocaleString()}`);
  console.log(`전체 DB 합:    ${totalDb.toLocaleString()} (${(totalDb / totalApi * 100).toFixed(1)}%)`);
  console.log(`raw gap (api-db): ${totalGap.toLocaleString()}`);
  console.log(`정상 월 수 (≥80%): ${rows.length - incomplete.length}`);
  console.log(`미완료 월 수 (<80%): ${incomplete.length}\n`);

  console.log("=== 미완료 월 (매칭 < 80%) ===");
  console.log("월       | G2B     | DB      | 매칭   | 누락");
  console.log("---------|---------|---------|--------|--------");
  for (const r of incomplete) {
    const miss = Math.max(0, r.api - r.db);
    console.log(
      `${r.ym} | ${String(r.api).padStart(7)} | ${String(r.db).padStart(7)} | ${r.ratio.toFixed(1).padStart(5)}% | ${String(miss).padStart(7)}`,
    );
  }

  const outPath = path.resolve(__dirname, "../../../../verify-totalcount-deadline-result.json");
  fs.writeFileSync(outPath, JSON.stringify({
    generatedAt: new Date().toISOString(),
    inqryDiv: 2,
    summary: {
      totalApi, totalDb, totalGap,
      matchRate: totalDb / totalApi,
      incompleteCount: incomplete.length,
    },
    incomplete: incomplete.map((r) => ({ ...r, gap: Math.max(0, r.api - r.db) })),
    allMonths: rows,
  }, null, 2));
  console.log(`\nJSON 저장: ${outPath}`);
