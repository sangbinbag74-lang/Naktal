  ...
  return (
    <div style={{ background: "#fff", borderRadius: 12, border: "2px solid #C7D2FE", padding: "20px 24px" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12, marginBottom: 20 }}>
        {[
          { label: "AI 추천 투찰률", value: hasData ? `${recommendedRate?.toFixed(4)}%` : "-", color: "#1B3A6B" },
          { label: "추천 금액", value: hasData && recommendedAmount ? `${new Intl.NumberFormat("ko-KR").format(recommendedAmount)}원` : "-", color: "#0F172A" },
          { label: "발주처 낙찰률", value: hasData && avgBidRate ? `${avgBidRate.toFixed(4)}%` : "-", color: "#059669" },
        ].map((item) => (
      ...
      {hasData && confidenceLow != null && confidenceHigh != null && (
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 11, color: "#94A3B8", marginBottom: 6 }}>
            <span>하한율 {confidenceLow.toFixed(4)}%</span>
            <span>추천 {recommendedRate?.toFixed(4)}%</span>
            <span>상한 {confidenceHigh.toFixed(4)}%</span>
          </div>
