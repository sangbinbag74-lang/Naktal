 ...
 */
import { MAIN_CNSTWK_MAP } from "@/lib/category-map";
...
export function g2bGetCategory(item: G2BAnnouncement, operation: string): string {
  if (operation === "getBidPblancListInfoCnstwk") {
    const mainCnstty = item.mainCnsttyNm?.trim();
    if (mainCnstty) {
      return MAIN_CNSTWK_MAP[mainCnstty] ?? "시설공사";
    }
    return "시설공사"; // ntceKindNm은 category에 절대 사용 금지
  }
  return item.pubPrcrmntMidClsfcNm || item.pubPrcrmntLrgClsfcNm || "기타";
}
